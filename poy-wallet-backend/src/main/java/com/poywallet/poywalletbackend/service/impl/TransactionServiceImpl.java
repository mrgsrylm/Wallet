...
import com.poywallet.poywalletbackend.mapper.TransactionResponseMapper;
import com.poywallet.poywalletbackend.mapper.TransactionRequestMapper;
import com.poywallet.poywalletbackend.model.Transaction;
import com.poywallet.poywalletbackend.repository.TransactionRepository;
...
public class TransactionServiceImpl implements TransactionService {
    ...
    @Override
    public Page<ResponseTransaction> findAll(Pageable pageable) {
        final Page<ResponseTransaction> transactions = transactionRepository.findAll(pageable)
    ...
    @Transactional(readOnly = true)
    @Override
    public List<ResponseTransaction> findAllByUserId(Long userId) {
        final List<ResponseTransaction> transactions = transactionRepository.findAllByUserId(userId).stream()
                .map(transactionResponseMapper::toDto)
                .toList();
    ...
    @Transactional(readOnly = true)
    @Override
    public ResponseTransaction findById(long id) {
    ...
    @Transactional(readOnly = true)
    @Override
    public ResponseTransaction findByReferenceNumber(UUID referenceNumber) {
     ...
     */
    @Override
    public ResponseCommand create(RequestTransaction request) {
        ...
        return ResponseCommand.builder().id(transaction.getId()).build();
    }
}
