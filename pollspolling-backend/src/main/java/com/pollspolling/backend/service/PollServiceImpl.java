...
import com.pollspolling.backend.domain.entity.Choice;
...
import com.pollspolling.backend.domain.mapper.ModelMapper;
...
public class PollServiceImpl implements PollService {
    ...
    private Map<Long, Long> getPollUserVoteMap(UserPrincipal currentUser, List<Long> pollIds) {
        // Retrieve Votes done by the logged user to the given pollIds
        Map<Long, Long> pollUserVoteMap = null;
        if(currentUser != null) {
            List<Vote> userVotes = voteRepository.findByUserIdAndPollIdIn(currentUser.getId(), pollIds);

            pollUserVoteMap = userVotes.stream().collect(Collectors.toMap(vote ->
                    vote.getPoll().getId(), vote -> vote.getChoice().getId()));
        }
