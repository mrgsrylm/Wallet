...
const TABLE_HEAD = [
  ...
  { id: 'form_wallet', label: 'Sender', alignRight: false },
  { id: 'to_wallet', label: 'Receiver', alignRight: false },
  { id: 'amount', label: 'Amount', alignRight: true },
  { id: 'description', label: 'Description', alignRight: false },
  { id: 'created_at', label: 'Time of Transaction', alignRight: false },
                ...
                <TableBody>
                  {data &&
                    data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                      const { id, amount, description, created_at, from_wallet, to_wallet, transaction_type, status } = row;
                      ...
                      return (
                        <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedRecord}>
                          ...
                          <TableCell align="left">{`${from_wallet.user.firstName} ${from_wallet.user.lastName}`}</TableCell>
                          <TableCell align="left">{`${to_wallet.user.firstName} ${to_wallet.user.lastName}`}</TableCell>
                          <TableCell align="right">{amount}</TableCell>
                          <TableCell align="left">{description}</TableCell>
                          <TableCell align="left">{created_at}</TableCell>
                          <TableCell align="left">{transaction_type}</TableCell>
